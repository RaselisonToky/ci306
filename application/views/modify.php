...
<form action="<?=base_url('myproduct/tenaModify?idp='.$pd[0]['idproduit'] )?>" method="post" enctype="multipart/form-data" >
    <div style="margin-left: 500px; margin-top: 200px">
        <div>
            <input type="text" name="nomProduit" placeholder="Votre Produit" value="<?=$pd[0]['nomproduit'] ?>">
            <input type="text" name="prix" placeholder="Prix Produit" value="<?=$pd[0]['estimation'] ?>">
        </div>
        <div style="margin-top: 50px">
            <select name="categorie">
                <?php
                for ($i=0;$i<count($allcategorie);$i++){ ?>
                    <option value="<?=$allcategorie[$i]['idcategorie'] ?>"><?=$allcategorie[$i]['nomcategorie'] ?></option>
                <?php } ?>
            </select>
        </div>
        <div style="margin-top: 50px">
            <input type="file" name="image_file"  enctype="multipart/form-data">
        </div>
